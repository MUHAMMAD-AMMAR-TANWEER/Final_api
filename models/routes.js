...
router.get("/data/:Device", async (req, res) => {
    ...
    try {
        const post = await Post.findOne({ Device: req.params.Device }, { customer: 0 })
        res.send(post)
    } catch {
        res.status(404)
...
router.post("/signup", async (req, res) => {
    ...
    await customer.save().then(customer => { res.status(200).send({ message: "User Registered" }) }).catch((err => res.status(400).send({ message: "Email Already registed" })));
...
router.post("/addDevice", async (req, res) => {
    ...
    const customer = await Customer.findOne({
        ...
        if (!customer) {
            ...
            newPost.save().then(newPost => { res.status(200).send({ message: "Device Registered" }) }).catch((err => res.status(400).send({ message: "Can register One-device per account now" })));
        }
    })
...
router.post("/addPattern", async (req, res) => {
    ...
    if (req.body.Pattern == "pattern1") {
        ...
        }).then(updating_pattern => { res.status(200).send({ message: "Pattern is updated" }) }).catch((err => res.status(400).send({ message: "Something went wrong" })));
...
router.post("/addSchedule", async (req, res) => {
    ...
    if (req.body.Pattern == "pattern1") {
        ...
        }).then(updating_pattern => { res.status(200).send({ message: "Schedule  is set" }) }).catch((err => res.status(400).send({ message: "Something went wrong" })));
    }
